...
%% Fourier transform
EPITH = fft2(epith);
figure(1);colormap(gray);subplot(1,2,1);imagesc(epith);
subplot(1,2,2);imagesc(fftshift(log(abs(EPITH))));

%% Inverse Fourier (sanity check)
epith_reconstructed = ifft2(EPITH);
figure(2);colormap(gray);subplot(1,2,1);imagesc(epith);
subplot(1,2,2);imagesc(epith_reconstructed);
...
figure(3);colormap(gray);imagesc(fftshift(log(abs(FILTER))));
...
figure(4);colormap(gray);subplot(1,2,1);imagesc(fftshift(log(abs(EPITH_FILTERED))));
subplot(1,2,2);imagesc(abs(epith_filtered));
